...
namespace MongoManager
{
    ...
    {
        std::optional<bsoncxx::document::value> doc = bans().find_one(
            stream_document() << "guildId" << guildId << "userId" << userId << finalize);
        if (doc)
    ...
    {
        std::optional<bsoncxx::document::value> doc = chills().find_one(
            stream_document() << "channelId" << channelId << "guildId" << guildId << finalize);
        if (doc)
    ...
    DbGang fetchGang(std::string_view name, int64_t guildId)
    {
        ...
        opts.collation(stream_document() << "locale" << "en" << "strength" << 2 << finalize);

        std::optional<bsoncxx::document::value> doc = gangs().find_one(
            stream_document() << "guildId" << guildId << "name" << name << finalize, opts);
        return doc ? DbGang(doc.value()) : DbGang();
    }

    DbPot fetchPot(int64_t guildId)
    {
        std::optional<bsoncxx::document::value> doc = pots().find_one(stream_document() << "guildId" << guildId << finalize);
        if (doc)
    ...
    {
        std::optional<bsoncxx::document::value> doc = users().find_one(
            stream_document() << "userId" << userId << "guildId" << guildId << finalize);
        if (doc)
    ...
    {
        std::optional<bsoncxx::document::value> doc = channelConfigs().find_one(
            stream_document() << "guildId" << guildId << finalize);
        if (doc)
    ...
    {
        std::optional<bsoncxx::document::value> doc = miscConfigs().find_one(
            stream_document() << "guildId" << guildId << finalize);
        if (doc)
    ...
    {
        std::optional<bsoncxx::document::value> doc = rankConfigs().find_one(
            stream_document() << "guildId" << guildId << finalize);
        if (doc)
    ...
    {
        std::optional<bsoncxx::document::value> doc = roleConfigs().find_one(
            stream_document() << "guildId" << guildId << finalize);
        if (doc)
    ...
    {
        bans().find_one_and_replace(stream_document() << "guildId" << ban.guildId << "userId" << ban.userId << finalize,
                                    ban.toDocument());
    }

    void updateChill(const DbChill& chill)
    {
        chills().find_one_and_replace(stream_document() << "channelId" << chill.channelId << "guildId" << chill.guildId << finalize,
                                      chill.toDocument());
    }
    ...
    void updateGang(const DbGang& gang)
    {
        ...
        opts.collation(stream_document() << "locale" << "en" << "strength" << 2 << finalize);
        gangs().find_one_and_replace(stream_document() << "guildId" << gang.guildId << "name" << gang.name << finalize,
                                     gang.toDocument());
    }

    void updatePot(const DbPot& pot)
    {
        pots().find_one_and_replace(stream_document() << "guildId" << pot.guildId << finalize, pot.toDocument());
    }

    void updateUser(const DbUser& user)
    {
        users().find_one_and_replace(stream_document() << "userId" << user.userId << "guildId" << user.guildId << finalize,
                                     user.toDocument());
    }

    void updateChannelConfig(const DbConfigChannels& config)
    {
        channelConfigs().find_one_and_replace(stream_document() << "guildId" << config.guildId << finalize, config.toDocument());
    }

    void updateGlobalConfig(const DbConfigGlobal& config)
    {
        globalConfig().find_one_and_replace({}, config.toDocument());
    }

    void updateMiscConfig(const DbConfigMisc& config)
    {
        miscConfigs().find_one_and_replace(stream_document() << "guildId" << config.guildId << finalize, config.toDocument());
    }

    void updateRankConfig(const DbConfigRanks& config)
    {
        rankConfigs().find_one_and_replace(stream_document() << "guildId" << config.guildId << finalize, config.toDocument());
    }

    void updateRoleConfig(const DbConfigRoles& config)
    {
        roleConfigs().find_one_and_replace(stream_document() << "guildId" << config.guildId << finalize, config.toDocument());
    }
