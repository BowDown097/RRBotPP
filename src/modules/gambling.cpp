...
dpp::task<dpp::command_result> Gambling::dice(const cash_in& betIn, int number)
{
    ...
    case 2:
        payout = bet;
        description += std::format(Responses::Dice2Matches, RR::utility::currencyToStr(bet * 2));
        break;
    case 3:
        payout = bet * 9;
        description += std::format(Responses::Dice3Matches, RR::utility::currencyToStr(bet * 10));
        break;
    default:
        description += std::format(Responses::DiceNoMatches, RR::utility::currencyToStr(bet));
        break;
    }

    long double totalCash = user.cash + payout;
    description += "\nBalance: " + RR::utility::currencyToStr(totalCash);
    ...
    if (user.gamblingMultiplier > 1)
    {
        ...
        description += std::format("\n*(+{} from gambling multiplier)*", RR::utility::currencyToStr(multiplierCash));
...
dpp::task<dpp::command_result> Gambling::pot(const std::optional<cash_in>& betIn)
{
    if (!betIn)
    {
        ...
        dpp::embed embed = dpp::embed()
            .set_color(dpp::colors::red)
            .set_title("Pot")
            .add_field("Total Value", RR::utility::currencyToStr(pot.value))
        ...
        for (const auto& [userId, bet] : pot.members)
        {
            if (dpp::user* user = dpp::find_user(userId))
                memberInfo += std::format("**{}**: {} ({:.2f}%)\n", user->get_mention(), RR::utility::currencyToStr(bet), pot.getMemberOdds(userId));
            else
                memberInfo += std::format("**???**: {} ({:.2f}%)\n", RR::utility::currencyToStr(bet), pot.getMemberOdds(userId));
        }
...
dpp::task<dpp::command_result> Gambling::genericGamble(long double bet, long double odds, long double mult, bool exactRoll)
{
    ...
    if (!exactRoll ? roll >= odds : roll == odds)
    {
        ...
        std::string message = std::format(Responses::GenericGambleSuccess, roll,
                                          RR::utility::currencyToStr(payout), RR::utility::currencyToStr(totalCash));
...
void Gambling::statUpdate(DbUser& user, bool success, long double gain)
{
    ...
    else
    {
        user.mergeStats(std::unordered_map<std::string, std::string> {
            ...
            { "Net Gain/Loss from Gambling", RR::utility::currencyToStr(-gain) }
        });
    }
}
