...
Moderation::Moderation() : dpp::module<Moderation>("Moderation", "This is like George Orwell's, uh book, 1984.")
{
...
dpp::task<dpp::command_result> Moderation::ban(const dpp::guild_member& member,
                                               ...
                                               const std::optional<dpp::remainder<std::string>>& reason)
{
    ...
    if (roles.memberIsStaff(member))
        co_return dpp::command_result::from_error(std::format(Responses::CantDoActionOnStaff, "ban", member.get_mention()));
    ...
    if (duration.has_value())
    {
        if (long time = RR::utility::resolveDurationString(duration.value()))
        {
            ...
            if (conf.is_error())
                co_return dpp::command_result::from_error(std::format(Responses::ActionFailed, "Ban", conf.get_error().human_readable));
        }
        else
        {
            co_return dpp::command_result::from_error(Responses::InvalidDuration);
        }
    }
    else
    {
...
dpp::task<dpp::command_result> Moderation::chill(const std::string& duration)
{
    if (long time = RR::utility::resolveDurationString(duration))
    {
        if (time < Constants::ChillMinSeconds)
            co_return dpp::command_result::from_error(std::format(Responses::InvalidChillDuration, "less", RR::utility::formatSeconds(Constants::ChillMinSeconds)));
        if (time > Constants::ChillMaxSeconds)
            co_return dpp::command_result::from_error(std::format(Responses::InvalidChillDuration, "more", RR::utility::formatSeconds(Constants::ChillMaxSeconds)));
        ...
        if (const dpp::channel* channel = dpp::find_channel(context->msg.channel_id))
        {
            ...
            if (it == overwrites.end())
                co_return dpp::command_result::from_error(Responses::GetEveryoneOverwriteFailed);
            if (it->deny.has(dpp::permissions::p_send_messages))
                co_return dpp::command_result::from_error(Responses::ChannelAlreadyChilled);
            ...
            if (conf.is_error())
                co_return dpp::command_result::from_error(std::format(Responses::ActionFailed, "Chill", conf.get_error().human_readable));

            co_return dpp::command_result::from_success(std::format(Responses::ChannelChilled, RR::utility::formatSeconds(time)));
        }
...
dpp::task<dpp::command_result> Moderation::hackban(uint64_t userId, const std::optional<dpp::remainder<std::string>>& reason)
{
    dpp::confirmation_callback_t conf = co_await cluster->co_guild_ban_add(context->msg.guild_id, userId);
    if (conf.is_error())
        co_return dpp::command_result::from_error(std::format(Responses::ActionFailed, "Hackban", conf.get_error().human_readable));
...
dpp::task<dpp::command_result> Moderation::kick(const dpp::guild_member& member,
                                                const std::optional<dpp::remainder<std::string>>& reason)
{
    ...
    DbConfigRoles roles = MongoManager::fetchRoleConfig(member.guild_id);
    if (roles.memberIsStaff(member))
        co_return dpp::command_result::from_error(std::format(Responses::CantDoActionOnStaff, "kick", member.get_mention()));

    dpp::confirmation_callback_t conf = co_await cluster->co_guild_member_kick(member.guild_id, member.user_id);
    if (conf.is_error())
        co_return dpp::command_result::from_error(std::format(Responses::ActionFailed, "Kick", conf.get_error().human_readable));
...
dpp::task<dpp::command_result> Moderation::mute(const dpp::guild_member& member, const std::string& duration,
                                                const std::optional<dpp::remainder<std::string>>& reason)
{
    if (member.user_id == context->msg.author.id)
        co_return dpp::command_result::from_error(Responses::BadIdea);
    if (member.is_communication_disabled())
        co_return dpp::command_result::from_error(std::format(Responses::UserAlreadyMuted, member.get_mention()));
    ...
    DbConfigRoles roles = MongoManager::fetchRoleConfig(member.guild_id);
    if (roles.memberIsStaff(member))
        co_return dpp::command_result::from_error(std::format(Responses::CantDoActionOnStaff, "mute", member.get_mention()));

    if (long time = RR::utility::resolveDurationString(duration))
    {
        if (time > RR::utility::secondsInDays(28))
            co_return dpp::command_result::from_error(Responses::MuteTooLong);
        ...
        if (conf.is_error())
            co_return dpp::command_result::from_error(std::format(Responses::ActionFailed, "Mute", conf.get_error().human_readable));
...
dpp::task<dpp::command_result> Moderation::purge(int limit)
{
    if (limit <= 0)
        co_return dpp::command_result::from_error(Responses::BadIdea);
    if (limit > 100)
        co_return dpp::command_result::from_error(Responses::PurgeTooManyMessages);
    ...
    if (messages.empty())
        co_return dpp::command_result::from_error(Responses::PurgeFoundNoMessages);

    dpp::confirmation_callback_t delConf = co_await cluster->co_message_delete_bulk(messages, context->msg.channel_id);
    if (delConf.is_error())
        co_return dpp::command_result::from_error(std::format(Responses::ActionFailed, "Purge", delConf.get_error().human_readable));

    co_return dpp::command_result::from_success(std::format(Responses::PurgeSuccess, messages.size() - 1));
}

dpp::task<dpp::command_result> Moderation::purgeRange(uint64_t from, uint64_t to)
{
    ...
    if (get2Conf.is_error())
        co_return dpp::command_result::from_error(std::format(Responses::ActionFailed, "Purge", get2Conf.get_error().human_readable));
...
dpp::task<dpp::command_result> Moderation::purgeUser(const dpp::guild_member& member, const std::optional<int>& limitIn)
{
    ...
    co_return dpp::command_result::from_success(std::format(Responses::PurgeSuccess, messages.size()));
}

dpp::task<dpp::command_result> Moderation::unban(uint64_t userId)
{
    dpp::confirmation_callback_t conf = co_await cluster->co_guild_ban_delete(context->msg.guild_id, userId);
    if (conf.is_error())
        co_return dpp::command_result::from_error(Responses::UnbanFailed);

    MongoManager::deleteBan(userId, context->msg.guild_id);
    if (const dpp::user* user = dpp::find_user(userId))
        co_return dpp::command_result::from_success("Unbanned " + user->get_mention() + '.');
    else
        co_return dpp::command_result::from_success("Unbanned the user with that ID.");
}

dpp::task<dpp::command_result> Moderation::unchill()
{
    if (const dpp::channel* channel = dpp::find_channel(context->msg.channel_id))
    {
        std::vector<dpp::permission_overwrite> overwrites = channel->permission_overwrites;
        ...
        if (it == overwrites.end())
            co_return dpp::command_result::from_error(Responses::ChannelNotChilled);
        ...
        dpp::confirmation_callback_t conf = co_await cluster->co_channel_edit_permissions(*channel, it->id, it->allow, it->deny, false);
        if (conf.is_error())
            co_return dpp::command_result::from_error(std::format(Responses::ActionFailed, "Unchill", conf.get_error().human_readable));

        co_return dpp::command_result::from_success(Responses::ChannelThawed);
    }

    co_return dpp::command_result::from_error(Responses::GetChannelFailed);
}

dpp::task<dpp::command_result> Moderation::unmute(const dpp::guild_member& member)
{
    if (!member.is_communication_disabled())
    ...
    if (conf.is_error())
        co_return dpp::command_result::from_error(std::format(Responses::ActionFailed, "Unmute", conf.get_error().human_readable));

    co_return dpp::command_result::from_success(std::format(Responses::UnmutedUser, member.get_mention()));
}
