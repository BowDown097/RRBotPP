...
dpp::task<dpp::command_result> Gangs::buyVault()
{
    ...
    co_return dpp::command_result::from_success(std::format(Responses::VaultUnlocked, RR::utility::currencyToStr(Constants::GangVaultCost)));
}
...
dpp::task<dpp::command_result> Gangs::createGang(const dpp::remainder<std::string>& name)
{
    ...
    co_return dpp::command_result::from_success(std::format(Responses::GangCreated,
        *name, RR::utility::currencyToStr(Constants::GangCreationCost)));
}
...
dpp::task<dpp::command_result> Gangs::deposit(const cash_in& amountIn)
{
    ...
    if (amount < Constants::TransactionMin)
        co_return dpp::command_result::from_error(std::format(Responses::DepositTooLow, RR::utility::currencyToStr(amount)));
    ...
    co_return dpp::command_result::from_success(std::format(Responses::DepositedIntoVault,
        RR::utility::currencyToStr(finalAmount), Constants::VaultTaxPercent));
}
...
dpp::command_result Gangs::gang(const std::optional<dpp::remainder<std::string>>& nameIn)
{
    ...
    if (gang.vaultBalance >= 0.01L)
        embed.add_field("Vault Balance", RR::utility::currencyToStr(gang.vaultBalance));
...
dpp::command_result Gangs::vaultBalance()
{
    ...
    return dpp::command_result::from_success(std::format(Responses::GangVaultBalance, RR::utility::currencyToStr(gang.vaultBalance)));
}
...
dpp::task<dpp::command_result> Gangs::withdrawVault(const cash_in& amountIn)
{
    long double amount = amountIn.top_result();
    if (amount < Constants::TransactionMin)
        co_return dpp::command_result::from_error(std::format(Responses::WithdrawTooLow, RR::utility::currencyToStr(Constants::TransactionMin)));
    ...
    co_return dpp::command_result::from_success(std::format(Responses::WithdrewFromVault, RR::utility::currencyToStr(amount)));
}
